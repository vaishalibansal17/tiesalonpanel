        ...
        value: function getUserProfile() {
          ...
          this.httpService.getRequest('GET_PARMS', 'PROMO_DETAIL', this.id, '').subscribe(function (response) {
            if (response.status === 1) {
              ...
              _this.detail.service = _this.detail.service.map(function (y) {
                return y.cat_name;
              }).join(', ');
            } else {
              if (response.err) {

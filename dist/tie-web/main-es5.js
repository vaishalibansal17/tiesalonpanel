        ...
        value: function getUserProfile() {
          ...
          this.httpService.getRequest('GET_PARMS', 'BOOKING_VIEW', this.id, '').subscribe(function (response) {
            if (response.status === 1) {
              ...
              _this.detail['slnme'] = JSON.parse(localStorage.getItem('salon')).name; // this.ttlamt = this.detail.service.length == 1 ? this.ttlamt.cost : this.ttlamt;

              _this.detail.service.map(function (v, i) {
                _this.detail.service[i]['discprice'] = v.cost - v.cost * v.discount / 100;
              });

              _this.itmttlamt = _this.detail.service.reduce(function (acc, val) {
                return acc.discprice + val.discprice;
              });
              _this.itmttlamt = _this.detail.service.length == 1 ? _this.detail.service[0].discprice : _this.itmttlamt;
              _this.ttlamt = _this.detail.totalAmount;
            } else {
              if (response.err) {
      ...
      _createClass(Helper, [{
        ...
        value: function utcDate(data, time) {
          ...
          var date = new Date(data),
              mnth = ("0" + date.getMonth()).slice(-2),
              day = ("0" + date.getDate()).slice(-2);
          var m1 = moment_timezone__WEBPACK_IMPORTED_MODULE_2___default.a.utc([Number(date.getFullYear()), Number(mnth), Number(day)]); // var timeStamp = new Date(Number(date.getFullYear()), Number(mnth), Number(day), 0, 0, 0);
          // console.log(timeStamp, timeStamp.getTime());
          // let bac = moment.utc(timeStamp);
          // console.log(bac);

          if (isUTC) return m1.valueOf();
        }
      }, {
        key: "parsehhmm",

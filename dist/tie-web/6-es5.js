      ...
      function HeaderComponent(httpservice, router, message, trns) {
        ...
        this._unsubs = this.router.events.subscribe(function (event) {
          if (event instanceof _angular_router__WEBPACK_IMPORTED_MODULE_5__["NavigationStart"]) {
            ...
            _this.route = event.url == '/dashboard' || event.url == '/' ? "".concat(_this.trns.data["WELCOMETOOUR"]).concat(_this.detail ? ' ' + _this.detail.name + ' Admin' : '') : event.url.split('/')[1].includes('-') ? event.url.split('/')[1].replace('-', ' ') : event.url.split('/')[1];
          }
